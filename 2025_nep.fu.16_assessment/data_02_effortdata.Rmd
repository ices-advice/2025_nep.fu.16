...
```{r data eff, echo=TRUE, fig.height=7, fig.width=10, warning=FALSE, fig.cap="FU16 Irish Effort (Kw days) by year."}
#historic aggregated data SALY

eff <- read.csv("boot/data/effort/nep.fu.16.effort.accessions.csv",skip=1) #ignore first row as it had FU & 16 as titles)


neffplot <- ggplot(eff, aes(Year, kwdays)) + geom_line(size = 1) + theme_bw() +
             ylab("Effort in kW days ") + 
            coord_cartesian(ylim= c(0, 2000), xlim=c(1995,dat.year)) +
            scale_x_continuous(breaks = seq(1995, dat.year, by = 5))


neffplot + theme(axis.text=element_text(size=10),axis.title=element_text(size=10)) + 
  theme(legend.title=element_blank(), panel.grid=element_blank())

ggsave("data/data_02_irish_effortdata/FU16_effort_Kwdays.png")
...
```{r data ves, echo=TRUE, fig.height=7, fig.width=10, warning=FALSE, fig.cap="FU16 Number of vessels with landings > 10 t."}

ves <- read.csv("boot/data/effort/Number of vessel by year and FU.taf.csv")

ves <- ves %>% filter(FU =='16')
...
ggsave("data/data_02_irish_effortdata/FU16_number_vessels.png") # 42 vessels in 2024
```
